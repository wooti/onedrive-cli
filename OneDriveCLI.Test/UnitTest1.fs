...
let Test1 () =
    ...
    let doHash inp =
        let xorHash = new QuickXorHash ()
        let b = xorHash.ComputeHash(Convert.FromBase64String(inp))
    ...
    examples |> Seq.iter (fun (inp, outp) ->
        let xorHash = new QuickXorHash ()
        let b = xorHash.ComputeHash(Convert.FromBase64String(inp))
        Assert.AreEqual(outp, Convert.ToBase64String(b))
    )
